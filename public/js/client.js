...
function setButtonsTitle() {
    ...
    // left buttons
    tippy(shareRoomBtn, {
        content: 'Convide os outros para entrar',
    ...
    tippy(recordStreamBtn, {
        content: 'Gravar reunião',
        placement: 'right-start',
    });
    tippy(fullScreenBtn, {
        content: 'Ver em tela cheia',
        placement: 'right-start',
    });
    tippy(chatRoomBtn, {
    ...
    tippy(myHandBtn, {
        content: 'Levantar a mao para falar',
        placement: 'right-start',
    });
    tippy(whiteboardBtn, {
        content: 'Abrir lousa',
...
function whoAreYou() {
    ...
    Swal.fire({
        ...
        inputValidator: (value) => {
            if (!value) return 'por favor digite seu nome';
            ...
            myPeerName = value;
            myVideoParagraph.innerHTML = myPeerName + ' (eu)';
    ...
    tippy(initAudioBtn, {
        content: 'Clique para audio OFF',
        placement: 'top',
    });
    tippy(initVideoBtn, {
        content: 'Clique para video OFF',
        placement: 'top',
    });
}
...
function welcomeUser() {
    ...
    Swal.fire({
        ...
        html:
            `
        <br/> 
        <p style="color:white;">Compartilhe essa reunião para que outros possam participar.</p>
        <p style="color:rgb(8, 189, 89);">` +
            myRoomUrl +
            `</p>`,
        showDenyButton: true,
        showCancelButton: true,
        confirmButtonText: `Copiar URL da sala`,
        denyButtonText: `Convide por email`,
...
async function shareRoomUrl() {
    ...
    if (navigator.share) {
        ...
        try {
            ...
            userLog('toast', 'Endereço da sala copiado com sucesso!');
        } catch (err) {
    ...
    if (!isSupportedNavigatorShare || (isSupportedNavigatorShare && errorNavigatorShare)) {
        ...
        Swal.fire({
            ...
            <br/><br/>
            <p style="color:white;"> Compartilhe essa sala para que outros possam entrar.</p>
            <p style="color:rgb(8, 189, 89);">` +
                myRoomUrl +
                `</p>`,
            showDenyButton: true,
...
function handleAudio(e, init) {
    ...
    if (init) {
        ...
        if (!isMobileDevice) {
            tippy(initAudioBtn, {
                content: myAudioStatus ? 'Clique para audio OFF' : 'Clique para audio ON',
                placement: 'top',
            });
        }
    }
...
function handleVideo(e, init) {
    ...
    if (init) {
        ...
        if (!isMobileDevice) {
            tippy(initVideoBtn, {
                content: myVideoStatus ? 'Clique para video OFF' : 'Clique para video ON',
                placement: 'top',
            });
        }
    }
    ...
    try {
        ...
        userLog(
            ...
            `<div style="text-align: left;">
                🔴 Informação da gravação <br/>
                Arquivo: ${recFileName} <br/>
                Tamanho: ${blobFileSize} <br/>
                Aguarde para ser processado e o download será feito em seu ${currentDevice} dispositivo.
            </div>`,
        );
...
function setMyAudioStatus(status) {
    ...
    tippy(myAudioStatusIcon, {
        content: status ? 'Microfone Ligado' : 'Microfone Desligado',
    });
    status ? playSound('on') : playSound('off');
    // only for desktop
    if (!isMobileDevice) {
        tippy(audioBtn, {
            content: status ? 'Clique para audio OFF' : 'Clique para audio ON',
            placement: 'right-start',
        });
    }
}
...
function setMyVideoStatus(status) {
    ...
    tippy(myVideoStatusIcon, {
        content: status ? 'Camera Ligada' : 'Camera Desligada',
    });
    status ? playSound('on') : playSound('off');
    // only for desktop
    if (!isMobileDevice) {
...
function showAbout() {
    ...
    Swal.fire({
        background: swalackground,
        position: 'center',
        title: '<strong>WebRTC feito com ❤️</strong>',
        imageAlt: '',
        imageUrl: aboutImg,
        html: `
        <br/>
        <div id="about">
            ...
            <img alt="" src="../images/github.png"></a><br/><br/>
            <button class="far fa-heart pulsate" onclick="window.open('')"> Sponsor</button>
            <br /><br />
            Author:<a href="" target="_blank"> Raphael Williams</a>
        </div>
        `,
        showClass: {
            popup: 'animate__animated animate__fadeInDown',
